...
# Seed 7, 24 train, K=4, 12 steps, consume_put_batch. Default Stage-3 memory path.
# Eval shadows DFA on frozen 32-dev at steps 0/12. Independent checkpoint root.
# Lock: configs/e3_4b_fc_no_qr.json
...
if [[ "$TRINITY_MODEL_PATH" == *Qwen2.5-1.5B-Instruct* ]]; then
  printf 'Refusing the 1.5B model path; E3 no-QR requires the locked Qwen3-4B directory.\n' >&2
  exit 2
fi

repository_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
python_bin="${AGEMEM_PYTHON_BIN:-python}"
log_root="$TRINITY_CHECKPOINT_ROOT_DIR/e3_4b_fc_no_qr_logs/$AGEMEM_EXPECTED_COMMIT"
project_dir="$TRINITY_CHECKPOINT_ROOT_DIR/Trinity-RFT-AgeMem-M8"
e0_job="$project_dir/agemem-e0-4b-fc-e3-no-qr-eval"
train_job="$project_dir/agemem-e3-4b-fc-no-qr"
...
if [[ "$TRINITY_CHECKPOINT_ROOT_DIR" == *checkpoints-e1-4b-format-conditioned ]]; then
  printf 'Refusing the diagnosis checkpoint root; E3 no-QR needs /data/hjx/Age_mem/checkpoints-e3-4b-fc-no-qr.\n' >&2
  exit 2
fi
...
if [[ "$TRINITY_CHECKPOINT_ROOT_DIR" == *checkpoints-e3-4b-fc ]]; then
  printf 'Refusing the question-retrieve E3 checkpoint root; E3 no-QR needs /data/hjx/Age_mem/checkpoints-e3-4b-fc-no-qr.\n' >&2
  exit 2
fi
if [[ -e "$project_dir/agemem-e0-terminal-only-frozen-eval" || \
  ...
  printf 'Refusing a checkpoint root that already contains closed 1.5B/4B, diagnosis, pilot, question-retrieve, or E3 jobs.\n' >&2
  exit 2
fi
if [[ -e "$train_job" && ! -s "$trainer_receipt" ]]; then
  printf 'Refusing to reuse incomplete E3 no-QR job directory: %s\n' "$train_job" >&2
  exit 2
fi
...
if [[ -n "$(git status --porcelain)" ]]; then
  printf 'Refusing to run E3 no-QR on a dirty worktree.\n' >&2
  exit 2
fi
...
if lock["model"]["repository_id"] != "Qwen/Qwen3-4B":
    raise SystemExit("E3 no-QR repository_id drifted")
if lock["experiment_id"] != "e3_format_conditioned_4b_oracle_dfa_no_qr":
    raise SystemExit("E3 no-QR experiment_id drifted")
if lock["trainer_total_steps"] != TRAINER_TOTAL_STEPS:
    raise SystemExit("E3 no-QR must train 12 steps")
if lock.get("consume_put_batch") is not True:
    raise SystemExit("E3 no-QR must consume one explorer put_batch per trainer step")
if not lock.get("stage3_require_final_answer") or not lock.get("stage3_repair_untagged_answer"):
    raise SystemExit("E3 no-QR must enable Stage-3 answer nudges.")
if lock.get("stage3_question_retrieve") or lock.get("stage3_index_observed_context"):
    raise SystemExit("E3 no-QR must not index observed context or retrieve by question")
if lock.get("stage3_inject_gold_supporting"):
    raise SystemExit("E3 no-QR must not inject gold supporting facts")
if lock.get("reward_profile") != "terminal_dfa":
    raise SystemExit("E3 no-QR reward_profile must be terminal_dfa")
if _source_digest(TRAIN_YAML) != lock["source_files"]["train_config"]["sha256"]:
    raise SystemExit("E3 no-QR train YAML digest does not match the lock")
train_text = TRAIN_YAML.read_text(encoding="utf-8")
if f'name: "{TRAIN_JOB}"' not in train_text:
    raise SystemExit("E3 no-QR train YAML job name drifted")
if "reward_profile: terminal_dfa" not in train_text:
    raise SystemExit("E3 no-QR train YAML must use terminal_dfa")
if "e3_dfa_shadow: false" not in train_text:
    raise SystemExit("E3 no-QR train YAML must not shadow DFA")
if "stage3_question_retrieve: true" in train_text:
    raise SystemExit("E3 no-QR must not enable question-retrieve")
if "stage3_index_observed_context: true" in train_text:
    raise SystemExit("E3 no-QR must not index observed context")
if "stage3_question_retrieve: false" not in train_text:
    raise SystemExit("E3 no-QR train YAML must disable question-retrieve")
if "stage3_inject_gold_supporting: true" in train_text:
    raise SystemExit("E3 no-QR must not inject gold supporting facts")
if not selection_is_frozen(fc_lock):
    raise SystemExit(
        "E3 no-QR requires frozen 32-dev selection; "
        "the format-conditioned lock on this machine must already be frozen."
    )
eval_root = Path(os.environ["TRINITY_CHECKPOINT_ROOT_DIR"]) / "e3_4b_fc_no_qr_eval_yaml" / os.environ["AGEMEM_EXPECTED_COMMIT"]
...
"$python_bin" -m unittest tests.common.e3_4b_fc_no_qr_contract_test tests.common.e3_oracle_dfa_test

export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"

if ray status >/dev/null 2>&1; then
  printf 'A Ray cluster is already running; stop it before E3 no-QR.\n' >&2
  exit 2
fi
...
if [[ ! -s "$e0_receipt" ]]; then
  ...
  trinity run --config "$eval_dir/agemem_e0_4b_fc_e3_no_qr_eval.yaml" \
    2>&1 | tee "$log_root/e0_eval.log"
  if [[ ! -s "$e0_receipt" ]]; then
    printf 'E3 no-QR E0 did not persist bench_step_0_model_0.json\n' >&2
    exit 1
  fi
...
if [[ ! -s "$trainer_receipt" || ! -d "$train_job/global_step_12" ]]; then
  ...
  trinity run --config examples/agemem_hotpotqa/agemem_e3_4b_fc_no_qr.yaml \
    2>&1 | tee "$log_root/e3_no_qr_update.log"
  if [[ ! -s "$trainer_receipt" ]]; then
    printf 'E3 no-QR did not persist trainer_step_12.json\n' >&2
    exit 1
  fi
  if [[ ! -d "$train_job/global_step_12" ]]; then
    printf 'E3 no-QR did not persist global_step_12\n' >&2
    exit 1
  fi
  stop_ray "$log_root/ray_train_stop.log"
fi

eval_job="$project_dir/agemem-e3-4b-fc-no-qr-eval-s12"
eval_receipt="$eval_job/receipts/bench_step_0_model_0.json"
if [[ ! -s "$eval_receipt" ]]; then
  start_ray "$log_root/ray_eval_12_start.log"
  trinity run --config "$eval_dir/agemem_e3_4b_fc_no_qr_eval_s12.yaml" \
    2>&1 | tee "$log_root/e3_no_qr_eval_s12.log"
  if [[ ! -s "$eval_receipt" ]]; then
    printf 'E3 no-QR checkpoint eval did not persist bench_step_0_model_0.json for step 12\n' >&2
    exit 1
  fi
  stop_ray "$log_root/ray_eval_12_stop.log"
fi

printf 'Format-conditioned 4B E3 Oracle DFA without question-retrieve finished.\n'
printf 'Logs: %s\n' "$log_root"
