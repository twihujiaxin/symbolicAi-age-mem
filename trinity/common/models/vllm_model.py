...
# Compatibility: newer transformers removed all_special_tokens_extended from some tokenizers
# (e.g. Qwen2Tokenizer); vLLM still expects it. Patch base class before vLLM loads tokenizers.
try:
...
class vLLMRolloutModel(InferenceModel):
    ...
    async def chat(
        ...
        if messages[-1]["role"] == "assistant":
            try:
                ...
                # Log a warning to help diagnose which chat_template/messages combination failed.
                self.logger.warning(
                    "Failed to apply chat template with continue_final_message=True, "
                    "fallback to add_generation_prompt. Error: %s",
                    str(e),
                )
                prompt = self.tokenizer.apply_chat_template(
                    messages,
    ...
    async def run_api_server(self):
        ...
        # Bind the API server on 0.0.0.0 for broader reachability while
        # keeping the actual node IP for client connection URLs
        node_ip, port = self.get_available_address()
        self.api_server_host, self.api_server_port = node_ip, port
